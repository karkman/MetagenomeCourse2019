...
```
#Quick crash course into statistics of metagenomic data: Multivariate analysis and generalized linear models of overdispersed count data
...
Count data in metagenomes is almost always normalized to either library sizes or 16S rRNA gene counts to get rid off variation that is caused due to differences in library sizes or the number of bacteria. In our case we have a lot of non-bacterial DNA in our samples, and we are quatifying genes which are found exclusively in bacteria. The amount of bacterial DNA also varies between samples. This is why we will normalize our data to 16S counts.


```{r}
#Load in the ARG mapping result
ARG_bt<-as.matrix(read.table("/Users/kparnane/Documents/MetagenomeCourse2019/R_for_overdispersed_data/ARG_genemat.txt", fill= 1, header= T, row.names = 1, check.names = F))
...
#Read in metadata
sample_data<-read.table("/Users/kparnane/Documents/MetagenomeCourse2019/R_for_overdispersed_data/metadata.txt", sep="\t")
...
We  will use Phyloseq to organize our data.
...
Often the research question is wether there are differences in between all genes in samples that can be explained by an explanatory variable or variables. In this case multivariate methods can be used to calculate distances between samples using dissimilarity indexes and ordinating the samples. One of the most commonly used distances in ecology is the Bray-Curtis dissimilarity index. There are several distances sych as Euclidean, Jaccard and Horn-Morisita. However, usually is is best to use a method which does not take zero values into account. Ordination methods are used to ordinate samples in relation to each other in multiple dimensions. There are several ordination methods such as NDMS, PCA and PCoA. We will use PCoA.The first two dimensions or axes of the ordination can be used to plot the placement of samples in relation to each other in a two-dimensional space.
...
```
We can see that there are more purple points on the left. We can test if the difference we see visually is statistically significant using adonis function from 'vegan' package.
```{r}
...
Now based on our result we can conclude that there are no differences in the grouping of genes based on the explanatory variables. (The separation is borderline significant using apla of 0.05). If we would like to know which individual genes are different we could use DeSEQ to investigate fold changes between two groups or fit genes as vectors in the ordination using envfit.

##Analysis of diversity

In many studies there is interest in determining wether the diversity of bacteria or genes differs between groups. Again there are several diversity indeces, but in most cases  for metagenomic data it is best to use an indes which takes both evenness and the number of different taxa or genes into account.

Phyloseq has is able to plot diversities so we will use it.
...
ggplot(df, aes(y=SUM, x=DIET)) + geom_boxplot()
...
```
This looks pretty good if you just look at the the p-value, but the residual devience and AIC values, which indicate how well the model explains the data are big. This means that the model is not good at explaining the deviance,
...
Negative binomial distributions are an other option for overdispersed data. Negative binomial models are a big more advanced in modelling how the deviance is modelled.
```{r}
...
The over-dispersed Poisson (quasipoisson) and negative binomial models have different variance functions. One way to check which one may be more appropriate is to create groups based on the linear predictor, compute the mean and variance for each group, and finally plot the mean-variance relationship.
